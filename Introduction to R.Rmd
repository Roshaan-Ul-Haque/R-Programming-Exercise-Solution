...
```{r }
#### Start solution ####
f <- factor(c("Employed", "Unemployed", "Self-Employed"))
```
...
```{r }
#### Start solution ####
d <- data.frame(CustomerID = c("Alicia", "Nancy", "Fernando", "Louis", "Diane"), hasAccount= c("No", "Yes", "Yes", "Yes", "No"), totalBalance = c(NA, 10000, NA, 2000, NA))
d
```
```{r }
nrow(d)
...
answer <- print(paste("The GCD. of", num1,"and", num2,"is", gcd(num1, num2)))
```
```{r }
#### End solution ####
```
...
b <- as.integer(b)
ab <- a == b
allConditionsMeet <- paste("The value of a is", a, "and the value of b is", b, " - Condition =", ab)
allConditionsMeet
```
```{r }
print(paste("The value of a is", a, "and the value of b is", b, " - Condition =", ab))
```
```{r }
#### End solution ####
```
...
```
```{r }
as.POSIXct("2021-03-23")
```
```{r }
as.POSIXlt("2021-03-23")
```
```{r }
...
t0 <- as.POSIXct("2021-03-23 09:49:50", tz = "GMT")
t0
```
```{r }
curr_time-t0
...
```
```{r }
ma <- matrix(c(1:150),nrow=15, ncol=10)
ma
```
...
```{r }
mt <- c(rnorm(10), runif(10), rnorm(10,1))
mt1 <- gl(3, 10)
mt1
```
...
```{r }
circum_30 <- c(118,118,118)
circum_214 <- c(664)
```{r }
age_1 <- mean(circum_30)
age_1
```
```{r }
age_2 <- mean(circum_214)
age_2
```
```{r }
#### End solution ####
